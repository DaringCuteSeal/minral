...
RefMap generate_map() {
		RefMap refmap;
	for (int i=0; i<REF_MAP_RADIUS; i++) {
		for (int j=0; j<REF_MAP_RADIUS; j++) {
	...
	map_divide_recurse(rows_marked, 2, REF_MAP_RADIUS-2);

	// add mandatory ones
	rows_marked.push_back(0);
	rows_marked.push_back(REF_MAP_RADIUS-1);
	...
	for (int i = 0; i < int(rows_marked.size()); i++) {
		for (int a=0; a<REF_MAP_RADIUS; a++) {
		...
		for (int c=0; c<GetRandomValue(1, 5); c++) {
			int random_hit = GetRandomValue(0, REF_MAP_RADIUS-1);
			...
			while (refmap.data[rows_marked[i]+1][random_hit] == TUNNEL || refmap.data[rows_marked[i]+1][random_hit-1] == TUNNEL || refmap.data[rows_marked[i]+1][random_hit+1] == TUNNEL) {
				random_hit = GetRandomValue(0, REF_MAP_RADIUS-1);
			}
